...
class ObjDbRemote(serviceFinder: ServiceFinder,
    ...
    private var myObjDbActor: ObjDbActor? = null
    ...
    private val myDispatcher = messageDispatcherFactory.create(this).apply {
        addClass(ObjDbActor::class.java)
    }
    ...
    private val myMessageHandlerFactory = object : MessageHandlerFactory {
        override fun provideMessageHandler(connection: Connection?) =
                ObjDbActor(connection!!, this@ObjDbRemote, localName, myRepHost!!, myDispatcher, odbActorGorgel, mustSendDebugReplies)
    }
     ...
     */
    fun repositoryConnected(objDbActor: ObjDbActor?) {
        myObjDbActor = objDbActor
        if (objDbActor == null) {
            connectToRepository()
        } else {
    ...
    private fun newRequest(req: PendingRequest) {
        ...
        val currentOdbActor = myObjDbActor
