...
/**
 * Base class for both local and remote concrete implementations of the object
 * database interface.
 */
abstract class ObjectDatabaseBase(
        protected val gorgel: Gorgel,
        private val jsonToObjectDeserializer: JsonToObjectDeserializer) : ObjectDatabase {
    ...
    fun decodeObject(ref: String, results: Array<ObjectDesc>): Any? {
        ...
        return if (objStr == null) {
            gorgel.error("no object retrieved from object database for ref $ref")
            null
        } else {
            try {
    ...
    private inner class ClassDescReceiver(var myTag: String) : Consumer<Any?> {
        override fun accept(obj: Any?) {
            ...
            if (classes != null) {
                ...
                classes.useInObjectDatabase(this@ObjectDatabaseBase, gorgel)
            } else {
