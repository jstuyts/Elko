 ...
 *
 * Creating a [Context], [User], or [Item] object by the
 * context server may entail the creation of one or more mod objects (i.e.,
 * instances of some subclass of [Mod]) that are attached to it.
 ...
 *
 * To enable initialization operations based on the object environment, mods
 ...
 * into its containership hierarchy, the server will invoke the [objectIsComplete] method of each of that object's mods that implement this
 * interface.
 *
 * This interface is only useful when implemented by subclasses of [ ].
 *
 * IMPORTANT NOTE: Implementors of this interface MUST not depend on the
 * relative ordering of calls to [objectIsComplete] on different mods to
...
interface ObjectCompletionWatcher {
     ...
     * environment.
     *
     * This method will be called automatically by the server's object
     * creation mechanism upon completion of the creation of an object to which
     * the mod implementing this interface is attached.
     *
     * Note: if an object has more than one mod that implements this
