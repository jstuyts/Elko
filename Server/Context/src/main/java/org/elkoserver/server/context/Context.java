...
*    if not relevant.
    ...
    void forceClose(boolean dup) {
        amForceClosing = true;
        List<Deliverer> members = new LinkedList<>(myGroup.members());
     ...
     */
    private void sendContextDescription(Deliverer to, Referenceable maker) {
        String sess = null;
    ...
    public Deliverer neighbors(final Deliverer exclude) {
        return message -> sendToNeighbors(exclude, message);
    }
