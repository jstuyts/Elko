...
public abstract class Cap extends Mod implements ObjectCompletionWatcher, ClockUsingObject {
     ...
     *    encoded.
     */
    final void encodeDefaultParameters(JSONLiteral lit) {
        if (!amDeletable) {
     ...
     */
    public final void ensureValid(User from) throws MessageHandlerException {
     ...
     */
    @JSONMethod
    public final void delete(User from) throws MessageHandlerException {
        ensureReachable(from);
     ...
     */
    @JSONMethod({"dest"})
    public final void transfer(User from, String destRef)
        throws MessageHandlerException
    {
        ensureReachable(from);
        if (!amTransferrable) {
