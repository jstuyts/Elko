...
class Broker {
    ...
    Broker(Server server, Trace appTrace, Timer timer, TraceFactory traceFactory) {
        ...
        myWaiters = new HashMapMulti<>();
        myActors = new HashSet<>();
        myServiceWatchers = new HashSet<>();
        myLoadWatchers = new HashSet<>();
        ...
        if (myODB != null) {
            ...
            myODB.getObject("launchertable", null, obj -> {
                if (obj != null) {
     ...
     */
    private void noteServiceArrival(ServiceDesc service) {
     ...
     */
    private void noteServiceDeparture(ServiceDesc service) {
        JSONLiteral msg =
            AdminHandler.msgServiceDesc(myAdminHandler,
    ...
    void shutdownServer(boolean kill) {
        if (!kill) {
            for (BrokerActor actor : new LinkedList<>(myActors)) {
                actor.doDisconnect();
            }
        }
        if (myODB != null) {
    ...
    private class WaiterForService implements TimeoutNoticer {
         ...
         *
         * @param service  The name of the service sought.
         * @param waiter  The client who is waiting.
