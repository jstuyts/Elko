...
class LauncherTable implements Encodable {
     ...
     *
     */
    void setRunSettingOn(String component) {
        Launcher launcher = myLaunchers.get(component);
        if (launcher != null) {
            launcher.setRunSettingOn(false);
            amDirty = true;
        }
    }
    ...
    static class Launcher implements Encodable {
        ...
        String launch() {
            try {
                ...
                List<String> exploded = new LinkedList<>();
                while (parser.hasMoreTokens()) {
                    exploded.add(parser.nextToken());
                }
                new ProcessBuilder(exploded).start();
